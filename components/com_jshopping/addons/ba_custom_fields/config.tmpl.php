...
$standart_fields = '
	<tr class="control-dinamic">
		...
		<td>' . JHtml::_('select.genericlist', $field_type, 'dinamic_field[field_type][]', '', 'value', 'text', 0) . '</td>
		<td><input type="text" name="dinamic_field[title_admin][]" value="" placeholder="' . _JSHOP_BACF_TITLE_ADMIN . '" title="' . _JSHOP_BACF_TITLE_ADMIN_DESC . '" style="width: 190px;" /></td>
		<td><input type="text" name="dinamic_field[title_site][]" value="" placeholder="' . _JSHOP_BACF_TITLE_SITE . '" title="' . _JSHOP_BACF_TITLE_SITE_DESC . '" style="width: 190px;" /></td>
		<td><input type="number" min="0" step="1" name="dinamic_field[ordering][]" value="" placeholder="' . _JSHOP_BACF_ORDER . '" title="' . _JSHOP_BACF_ORDER_DESC . '" style="width: 60px;" /></td>
		<td><textarea name="dinamic_field[values_list][]" placeholder="' . _JSHOP_BACF_VALUES . '" title="' . _JSHOP_BACF_VALUES_DESC . '"></textarea></td>
		<td>' . JHtml::_('select.genericlist', $position_product, 'dinamic_field[position_product][]', '', 'value', 'text', 0) . '</td>
		<td>' . JHtml::_('select.genericlist', $position_category, 'dinamic_field[position_category][]', '', 'value', 'text', 0) . '</td>
		<td>' . JHtml::_('select.genericlist', $yes_no_options, 'dinamic_field[multilang[][]', 'class="chzn-color-state" style="width: 120px;"', 'value', 'text', 0) . '</td>
	</tr>
';
		...
		<tbody>
			...
			?>
						<tr class="control-dinamic">
							...
							<td><?php echo JHtml::_('select.genericlist', $field_type, 'dinamic_field[field_type][]', '', 'value', 'text', (isset($field->field_type) ? $field->field_type : 0)); ?></td>
							<td><input type="text" name="dinamic_field[title_admin][]" value="<?php echo (isset($field->title_admin) ? $field->title_admin : ''); ?>" placeholder="<?php echo _JSHOP_BACF_TITLE_ADMIN; ?>" title="<?php echo _JSHOP_BACF_TITLE_ADMIN_DESC; ?>" style="width: 190px;" /></td>
							<td><input type="text" name="dinamic_field[title_site][]" value="<?php echo (isset($field->title_site) ? $field->title_site : ''); ?>" placeholder="<?php echo _JSHOP_BACF_TITLE_SITE; ?>" title="<?php echo _JSHOP_BACF_TITLE_SITE_DESC; ?>" style="width: 190px;" /></td>
							<td><input type="number" min="0" step="1" name="dinamic_field[ordering][]" value="<?php echo (isset($field->ordering) ? $field->ordering : 0); ?>" placeholder="<?php echo _JSHOP_BACF_ORDER; ?>" title="<?php echo _JSHOP_BACF_ORDER_DESC; ?>" style="width: 60px;" /></td>
							<td><textarea name="dinamic_field[values_list][]" placeholder="<?php echo _JSHOP_BACF_VALUES; ?>" title="<?php echo _JSHOP_BACF_VALUES_DESC; ?>"><?php echo (isset($field->values_list) ? $field->values_list : ''); ?></textarea></td>
							<td><?php echo JHtml::_('select.genericlist', $position_product, 'dinamic_field[position_product][]', '', 'value', 'text', (isset($field->position_product) ? $field->position_product : 0)); ?></td>
							<td><?php echo JHtml::_('select.genericlist', $position_category, 'dinamic_field[position_category][]', '', 'value', 'text', (isset($field->position_category) ? $field->position_category : 0)); ?></td>
							<td><?php echo JHtml::_('select.genericlist', $yes_no_options, 'dinamic_field[multilang][]', 'class="chzn-color-state" style="width: 120px;"', 'value', 'text', (isset($field->multilang) ? $field->multilang : 0)); ?></td>
						</tr>
			<?php
					}
				} else {
