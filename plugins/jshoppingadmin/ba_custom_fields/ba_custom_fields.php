...
class plgJshoppingAdminBa_custom_fields extends JPlugin {
    ...
    public function onBeforeSaveAddons(&$params, &$post, &$row) {
        ...
        if ($post['dinamic_field']) {
            ...
            foreach($result_fields as $field) {
                ...
                if ($field['field_id'] == 0) {
                    $db->insertObject('#__jshopping_custom_fields', $field_data);
                    $field_id = $db->insertid();
                    $alter_text = 'ADD';
                } else {
                    $field_data->id = $field_id = $field['field_id'];
                    $db->updateObject('#__jshopping_custom_fields', $field_data, 'id');
    ...
    public function onDisplayProductEditTabsEndTab(&$row, &$lists, &$tax_value) {
        ...
        echo '<li><a href="#product_ba_custom_fields" class="nav-link" data-toggle="tab">Custom Fields</a></li>';
    }
    ...
    public function onDisplayProductEditTabsEnd(&$pane, &$row, &$lists, &$tax_value, &$currency) {
        ...
        echo '
			<style>
				...
				.custom_field_image > div > img,
				.gallery-block-list .custom_field_gallery > div > img {
				    width: 100%;
				    height: auto;
				    display: block;
				}
        ...
        if ($fields_list) {
            ...
            foreach($fields_list as $field) {
                ...
                if ($field->multilang) {
                    ...
                    echo '<tr>
							<td class="key">' . $field->title_admin . '</td>
							<td>' . $field_html . '</td>
						</tr>
					';
                }
        ...
        if (!empty($product->ba_custom_fields)) {
            ...
            foreach($languages as $lang) {
                ...
                if ($fields_product) {
                    ...
                    $db->updateObject('#__jshopping_custom_fields_values', $field_data, 'id');
                } else {
                    $db->insertObject('#__jshopping_custom_fields_values', $field_data);
                }
            }
        }
    }
						...
						<div class="custom_field_image image_block">
							...
							<div style="padding-bottom:5px;" class="link_delete_foto">
								<a class="btn btn-micro" href="#" onclick="deleteImageCustomField(jQuery(this)); return false;">
									<img src="components/com_jshopping/images/publish_r.png">' . \JText::_('JSHOP_DELETE') . '
								</a>
							</div>
